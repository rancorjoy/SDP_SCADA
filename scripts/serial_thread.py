...
def serial_loop(event_queue, is_init):                                              # Method is ran in entry point - returns "queue" of serial events
    ...
    while True:                                                                     # Continously while the main thread runs...
        ...
        if is_init:
            try:
                port_objects = serial.tools.list_ports.comports()                           # Get objects
                current_ports = set(p.device for p in port_objects if p.device)             # On Linux, some ports show up but aren't accessible.
                                                                                            # Filtering for valid descriptions or VID/PID can help.
